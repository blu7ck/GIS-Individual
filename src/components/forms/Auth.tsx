...
export const Auth: React.FC<Props> = ({ supabaseUrl, supabaseKey, onLogin, onError }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!supabaseUrl || !supabaseKey) {
      const errorMsg = 'Database connection missing. Cannot authenticate.';
      setError(errorMsg);
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (data.user) {
          ...
          // Sometimes signUp returns null user if confirm email is on
          throw new Error('Sign up successful but user creation pending confirmation.');
        }
      } else {
        // Sign In
      ...
      <div className="relative z-10 w-full max-w-[400px] p-8 mx-4 overflow-hidden rounded-2xl shadow-glass border border-white/10 backdrop-blur-xl bg-white/5">
        ...
        <div className="flex flex-col items-center mb-8 relative">
          ...
          <div className="mb-6 relative group">
            ...
            <div className="hidden h-20 w-20 bg-gradient-to-br from-accent-azure to-accent-cyan rounded-xl flex items-center justify-center text-3xl font-bold shadow-lg">F</div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="pt-2 flex gap-3">
            <Button
              ...
              className="flex-1 relative overflow-hidden group bg-gradient-to-r from-accent-azure to-blue-600 hover:from-blue-500 hover:to-accent-azure text-white border-none shadow-lg shadow-blue-500/20 py-3 transition-all duration-300 transform hover:-translate-y-0.5"
            >
              <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
              <span className="relative font-semibold tracking-wide flex items-center justify-center gap-2">
                {isLoading && !isSignUp ? 'Verifying...' : 'Login'}
              </span>
            </Button>
            ...
            <Button
              ...
              className="flex-1 border-white/10 hover:bg-white/5 text-white py-3 transition-all duration-300"
            >
              <span className="relative font-semibold tracking-wide flex items-center justify-center gap-2">
                {isLoading && isSignUp ? 'Creating...' : 'Sign Up'}
              </span>
            </Button>
          </div>
        </form>
